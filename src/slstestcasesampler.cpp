...
long long SLSTestcaseSampler::Get2TupleMapIndex(int i, int v_i, int j, int v_j)
{
    if (i >= j)
    {
    ...
    {
        int index_comb = v_j + v_i * 2;
        long long index = index_comb * num_combination_all_possible_ + (2 * num_var_ - i - 1) * i / 2 + j - i - 1;
        return index;
    }
}

long long SLSTestcaseSampler::Get3TupleMapIndex(int i, int v_i, int j, int v_j, int k, int v_k)
{
    if (i >= j || j >= k)
    {
    ...
    {
        int index_comb = v_k + v_j * 2 + v_i * 4;
        long long index = index_comb * num_combination_all_possible_ + (num_var_ * (i * num_var_ - (i + 2) * i) + i * (i + 1) * (i + 2) / 3) / 2 + (2 * num_var_ - i - j - 2) * (j - i - 1) / 2 + k - j - 1;
        return index;
    }
}
...
{
    num_combination_all_possible_ = num_var_ * (num_var_ - 1) * (num_var_ - 2) / 6;
    num_tuple_all_possible_ = num_combination_all_possible_ * 8;
    map_tuple_ = new int[num_tuple_all_possible_]();
    count_each_var_positive_uncovered_.resize(num_var_, (num_var_ - 1) * (num_var_ - 2) * 2);
    count_each_var_negative_uncovered_.resize(num_var_, (num_var_ - 1) * (num_var_ - 2) * 2);
    num_tuple_ = 0;
}
